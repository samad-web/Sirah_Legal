...
import { LayoutGrid, Folder, LogOut, ChevronRight, Scale, Briefcase } from 'lucide-react'
import { useAuth } from '@/contexts/AuthContext'
import { cn } from '@/lib/utils'

const clientNavItems = [
    { to: '/client/dashboard', icon: <LayoutGrid size={18} />, label: 'Dashboard' },
    { to: '/client/documents', icon: <Folder size={18} />, label: 'My Documents' },
]
            ...
            <AnimatePresence>
                {expanded && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        transition={{ duration: 0.15 }}
                        className="mx-2 mt-3 mb-1 px-3 py-1.5 bg-[#1B3A2D] border border-[rgba(201,168,76,0.25)] flex items-center gap-2"
                    >
                        <Briefcase size={12} className="text-[#C9A84C] shrink-0" />
                        <span className="text-[10px] text-[#C9A84C] tracking-widest whitespace-nowrap" style={{ fontFamily: 'DM Mono, monospace' }}>
                            CLIENT ACCESS
                        </span>
                    </motion.div>
                )}
                ...
                <AnimatePresence>
                    {expanded && profile && (
                        ...
                        >
                            ...
                            <p className="text-[10px] text-[rgba(201,168,76,0.4)] truncate mt-0.5" style={{ fontFamily: 'DM Mono, monospace' }}>
                                CLIENT PORTAL
                            </p>
                        </motion.div>
                    )}
